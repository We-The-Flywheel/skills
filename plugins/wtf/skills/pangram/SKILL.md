...
2. **Load the API key.**
   ...
   If `NOT_SET`, stop and tell the user to add `PANGRAM_API_KEY=<key>` to `~/.env.shared`.
...
- **401:** bad or missing API key — check `PANGRAM_API_KEY` in `~/.env.shared`
...
- The API key should live in `~/.env.shared` as `PANGRAM_API_KEY=<value>`
