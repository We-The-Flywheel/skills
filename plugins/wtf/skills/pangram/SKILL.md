...
- **Auth:** set `PANGRAM_API_KEY` in your environment (export it, or `source ~/.env.shared` if you keep it there)
...
2. **Confirm the API key is available.**
   ...
   If `NOT_SET`, stop and tell the user to set `PANGRAM_API_KEY` in their
   environment (a Pangram Labs API key from pangram.com) before re-running.
...
Verdict: Borderline. Run the humanizer skill on the flagged sections, then re-score.
```

## Error handling

- **401:** bad or missing API key — check that `PANGRAM_API_KEY` is set correctly
